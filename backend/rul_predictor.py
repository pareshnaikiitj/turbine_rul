...
def calculate_vibration_and_pressure(rpm: float, cycle: int, life: int, healthy: dict) -> tuple[float, float]:
    """Generate secondary sensor trends that rise with RPM and cumulative damage."""
    rpm_span = max(1.0, healthy["rpm"]["max"] - healthy["rpm"]["min"])
    rpm_ratio = (rpm - healthy["rpm"]["min"]) / rpm_span
    rpm_ratio = float(np.clip(rpm_ratio, 0.0, 1.0))
    degradation = cycle / max(1, life)

    vibration = round(healthy["vibration"]["min"] + 0.45 * rpm_ratio + 0.12 * degradation, 3)
    pressure = round(healthy["pressure"]["min"] + 0.35 * rpm_ratio + 0.15 * degradation, 3)
...
def load_and_store_real_data(uploaded_csv_path: str) -> pd.DataFrame:
    ...
    combined["vibration"] = combined["vibration"].fillna(
        combined.apply(lambda row: calculate_vibration_and_pressure(
            float(row["rpm"]), int(row["cycle"]), max(1, int(row["cycle"])), CONFIG["healthy_ranges"])[0], axis=1)
    )
    combined["pressure"] = combined["pressure"].fillna(
        combined.apply(lambda row: calculate_vibration_and_pressure(
            float(row["rpm"]), int(row["cycle"]), max(1, int(row["cycle"])), CONFIG["healthy_ranges"])[1], axis=1)
    )
...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["pressure_rise"] = df.groupby("unit_id")["pressure"].diff().fillna(0)
    ...
    for unit_id in unit_ids:
        ...
        for target_cycle in cycles:
            ...
            rows.append({
                ...
                "pressure": round(float(row.get("pressure", 0.0)), 3),
